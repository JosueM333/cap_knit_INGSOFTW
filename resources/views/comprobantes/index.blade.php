...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 fw-bold">Gestión de Comprobantes</h1>
    ...
    @if(session('info'))
        <div class="alert alert-info border-info text-center fw-bold mb-4"><i class="bi bi-info-circle me-2"></i> {{ session('info') }}</div>
    @endif
    @if(session('success'))
        <div class="alert alert-success border-success text-center fw-bold mb-4"><i class="bi bi-check-circle me-2"></i> {{ session('success') }}</div>
    @endif
    @if($errors->any())
        <div class="alert alert-danger border-danger text-center fw-bold mb-4">
            <ul class="mb-0 list-unstyled">
                @foreach($errors->all() as $error) <li><i class="bi bi-exclamation-triangle me-2"></i> {{ $error }}</li> @endforeach
            </ul>
        </div>
    @endif
        ...
        <div class="card-body px-4 pb-4">
            <form action="{{ route('comprobantes.buscar') }}" method="POST" class="row g-3 align-items-end">
                ...
                <div class="col-md-9">
                    ...
                    <input type="text" name="criterio" id="criterio" class="form-control form-control-lg" placeholder="Ingrese Número de Factura o Cédula..." required>
                </div>
                <div class="col-md-3 d-grid">
                    <button type="submit" class="btn btn-primary btn-lg fw-bold"><i class="bi bi-search me-1"></i> Buscar</button>
                </div>
            </form>
            @if(request()->routeIs('comprobantes.buscar'))
                <div class="mt-3"><a href="{{ route('comprobantes.index') }}" class="text-decoration-none fw-bold"><i class="bi bi-arrow-counterclockwise"></i> Ver todos los registros</a></div>
            @endif
        </div>
    </div>
    ...
    <div class="card shadow-sm border-0 mb-4 bg-light">
        <div class="card-body p-4 text-center border rounded d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('comprobantes.create') }}" class="btn btn-success btn-lg fw-bold px-4"><i class="bi bi-plus-circle me-2"></i> Nuevo Comprobante</a>
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="6" class="text-center text-muted py-5">No hay facturas emitidas.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="modal-content border-danger">
            <form id="formAnular" action="" method="POST">
                ...
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title fw-bold"><i class="bi bi-exclamation-triangle-fill me-2"></i> Anular Comprobante</h5>
